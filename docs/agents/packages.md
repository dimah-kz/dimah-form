...
- First-party `@dimah-form/dataset` is a read-side plugin (no `meta` namespace, no tables). Canonical interchange is JSONL + codebook from **response snapshots**. HTTP is paged (`getDatasetPage`); `getDatasetCodebook` is the historical union for the same filter (walk capped; query `maxRows` may lower the cap, not raise it; `truncated` when the cap hits). Canonical field / score views follow `lastSeenAt`; a differing prior snapshot is one `history` entry (field view, including scoring variable and option `points` / `add`, plus variable `missing` policy, bands, and formulas). `getLiveCodebook` is the live form only (`snapshots[].n` is 0). Full-file zip stays in the consumer app. Do not flatten against the live questionnaire. CSV writes a reserved field id as `field.<id>` and adds `score.<id>.missing`. `toDataPackage` is a Frictionless `data-package` (table schema on `responses.csv` only). Optional `onProject` upserts the projected record by `record.id` in the consumer warehouse.
- First-party `@dimah-form/insights` is a read-side plugin (no `meta` namespace, no tables). Compute-on-read status / completion / field stats (visibility-correct, categorical `%` and score bands in document order with unused `n: 0`, numeric for any finite number, date) from **response snapshots**. The live form is field order and catalog only — not a flatten. Optional day series, `whereField` segment (finite numbers match `String(value)`), and `GET /insights/crosstab` (`n` is respondents; a missing live axis still walks snapshots). Score bands need the optional scoring peer. Walks are capped (`truncated`). Do not persist summaries or add charts in this package.
