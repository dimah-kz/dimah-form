...
- Plugins may add `endpoints` (via `createFormEndpoint`), `hooks`, `fieldTypes`, `$ERROR_CODES` (`defineErrorCodes`), `metaSchema` / `metaNamespace`, `validateAnswers`, and phantom `$Meta` (`NamespacedMeta`).
- Optional `dependsOn` (topological `init` / hook / `validateAnswers` / `metaSchema` order), `options` (for sibling plugins), and synchronous `init` that may return `{ context }` stored on `config.pluginContext`.
- Guard `operation` for a plugin route is the `endpoints` key unless `metadata.operation` is set.
- Browser companions are `defineClientPlugin` merged in `createFormClient({ plugins })`. They are not inferred from the server plugin. Share `id` and the error catalog module. Client plugins may add `fieldTypes` (same definitions as the server plugin), `$ERROR_CODES`, `validateAnswers`, and `$Meta`.
