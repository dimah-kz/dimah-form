...
- Plugin `init` receives `basePath`, `fieldTypes`, sibling `plugins`, and `getPluginContext` — not the internal resolved config. The same `getPluginContext` is on every `on*` / `after*` hook. Persistence is `database`, not a plugin. Plugins must not add tables.
...
- First-party `@dimah-form/dataset` is a read-side plugin (no `meta` namespace, no tables) and depends on `@dimah-form/scoring` (import `@dimah-form/scoring/document` from isomorphic modules). Canonical interchange is JSONL + codebook from **response snapshots**. `snapshotKey` is SHA-256 of RFC 8785 canonical JSON of the instrument. HTTP is paged (`getDatasetPage`); `getDatasetCodebook` is the historical union for the same filter (walk capped; query `maxRows` may lower the cap, not raise it; `truncated` when the cap hits). Canonical field / score views follow `lastSeenAt`; a differing prior snapshot is one `history` entry (field view, including scoring variable and option `points` / `add`, plus variable `missing` policy, bands, and formulas). Scoring meta is read with `readScoringFormMeta` / `readScoringFieldMeta`. A scoring `APIError` on one row omits scores; other errors propagate. `getLiveCodebook` is the live form only (`snapshots[].n` is 0). Full-file zip stays in the consumer app. Do not flatten against the live questionnaire. CSV writes a reserved field id as `field.<id>` and adds `score.<id>.missing`. `attachment` is set only for `type: "file"`. `toDataPackage` is a Frictionless tabular data package (table schema, `primaryKey`, and dialect on `responses.csv` only). Optional `onProject` upserts the projected record by `record.id` in the consumer warehouse.
- First-party `@dimah-form/insights` is a read-side plugin (no `meta` namespace, no tables) and depends on `@dimah-form/scoring`. Compute-on-read status / completion / field stats (visibility-correct, categorical `%` and score bands in document order with unused `n: 0`, numeric for any finite number, date) from **response snapshots**. The live form is field order and catalog only — not a flatten. Optional day series (`timeZone` IANA, default `UTC`), `whereField` segment (finite numbers match `String(value)`), and `GET /insights/crosstab` (`n` is respondents). A crosstab axis counts only where that snapshot field is categorical — the live type does not reject the query. Walks are capped (`truncated`). Do not persist summaries or add charts in this package.
