...
Optional. Wrap `form` (`FormResponseApi`) and `binding` (`FormFieldBinding`). Widgets take `{ binding, className, mode? }` and put chrome on `FormFieldFrame`. Do not call `useFormResponse` inside widgets. Built-in type widgets register on `defaultFieldWidgets`; custom types use the `widgets` prop (same `type` string as `defineFieldType`). `FormUiProvider` `components.RequiredMark` replaces the required-field asterisk. Built-in widgets read `FieldUiMeta` on `field.meta` (`placeholder`, `widget`, `step`, `section`, `width`, …). Author with `defineForm({ ... } satisfies FormDefinitionUi)`. `FormView layout` is `auto` (steps when `meta.step` groups; review when locked). Step titles live on `form.meta.steps`. `FormSteps` must wrap `FormRoot` so Enter advances. Boolean `unsetOnOff` is a field document key. `session.errorCode` / `session.autosave` feed `FormError` and `FormActions save="auto"`. `validate(mode, { fields })` checks one wizard step.
