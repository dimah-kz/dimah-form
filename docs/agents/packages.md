...
- First-party `@dimah-form/insights` is a read-side plugin (no `meta` namespace, no tables). Compute-on-read status / completion / field stats (visibility-correct, categorical `%`, number / date) from **response snapshots**. Optional day series, `whereField` segment, and `GET /insights/crosstab`. Score bands need the optional scoring peer. Walks are capped (`truncated`). Do not persist summaries or add charts in this package.
