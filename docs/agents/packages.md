...
- First-party `@dimah-form/scoring` is a feature plugin (`meta.scoring`). Scores are derived from the response snapshot; do not persist them into `answers` or add scoring tables.
