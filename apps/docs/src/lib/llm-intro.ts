...
- Persistence is \`database\`, not a plugin. Plugins add endpoints, hooks, field types, error codes, \`metaSchema\`, \`validateAnswers\`, and \`validateDefinition\`. Official Likert scoring is \`@dimah-form/scoring\` (\`meta.scoring\` on the snapshot; do not store scores in \`answers\`). Official dataset interchange is \`@dimah-form/dataset\` (JSONL + codebook from the response snapshot; no tables). Official insights summaries are \`@dimah-form/insights\` (counts from the response snapshot; no tables).
...
- [@dimah-form/insights](${npmPackageUrls[7]}): optional insights plugin (read-side snapshot summaries)
