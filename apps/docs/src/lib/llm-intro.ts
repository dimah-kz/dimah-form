...
- Persistence is \`database\`, not a plugin. Plugins add endpoints, hooks, field types, error codes, \`metaSchema\`, \`validateAnswers\`, and \`validateDefinition\`. Official Likert scoring is \`@dimah-form/scoring\` (\`meta.scoring\` on the snapshot; do not store scores in \`answers\`).
