...
| **`@dimah-form/scoring`** | Server / isomorphic | Official Likert / subscale plugin. `meta.scoring` on the snapshot; compute-on-read totals and bands. Peer of `server`.                                                                           | `scoringPlugin()`, `scoreResponse()`, `scoringClientPlugin()`                |
