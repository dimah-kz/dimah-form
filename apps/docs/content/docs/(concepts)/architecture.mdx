...
| **`@dimah-form/insights`** | Server / isomorphic | Official insights plugin. Status / completion / field stats from response snapshots; optional day series and crosstab. Compute-on-read; no tables. Peer of `server`.                              | `insightsPlugin()`, `createInsightsAccumulator()`, `insightsClientPlugin()`  |
