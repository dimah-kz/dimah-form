...
| **`@dimah-form/insights`** | Server / isomorphic | Official insights plugin. Status / completion / categorical counts from response snapshots. Compute-on-read; no tables. Peer of `server`.                                                        | `insightsPlugin()`, `createInsightsAccumulator()`, `insightsClientPlugin()`  |
