...
```ts
{
  ...
  meta: {
    ...
    help: "We will send your onboarding packet here.",
  },
}
```
...
- **`meta`**: Opaque JSON for UI. `@dimah-form/ui` reads a typed `FieldUiMeta` bag: `placeholder`, `multiline`, `rows`, `widget` (`radio` / `switch` / `chips`), `section`, `step`, `stepTitle`, `width`, `prefix` / `suffix`, `autocomplete`, `inputMode`, `orientation`, `help`, `unsetOnOff`. See [UI](/docs/ui).
