...
We recommend defining each questionnaire in a dedicated file under `lib/forms/`. If you render with `@dimah-form/ui`, `satisfies FormDefinitionUi` types `meta` against that package:
...
- **`meta`**: Opaque JSON for UI. When you use `@dimah-form/ui`, type the document with `satisfies FormDefinitionUi` so `placeholder`, `widget`, `section`, `step`, `width`, … autocomplete. See [UI](/docs/ui). Boolean `unsetOnOff` is a field key, not meta.
