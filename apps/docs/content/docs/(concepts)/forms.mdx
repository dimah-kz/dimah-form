...
We recommend defining each questionnaire in a dedicated file under `lib/forms/`. If you render with `@dimah-form/ui`, `satisfies FormDefinitionUi` types `meta` against that package — skip it when you do not use `@dimah-form/ui`. Custom `defineFieldType` keys are a separate bind (`createDefineForm`); see [Custom fields](/docs/custom-fields).
...
```ts
{
  ...
  description: "We will send your onboarding packet here.",
  meta: {
...
- **`type`**: Built-in type (`"text"`, `"number"`, `"boolean"`, `"select"`, `"multiSelect"`, `"email"`, `"date"`, `"file"`) or a custom type registered via `defineFieldType`.
...
- **`description`**: Helper copy under the control in `@dimah-form/ui`.
- **`meta`**: Opaque JSON for UI. When you use `@dimah-form/ui`, type the document with `satisfies FormDefinitionUi` so `placeholder`, `widget`, `section`, `step`, `width`, … autocomplete (`FormDefinitionUi<typeof fieldTypes>` when the catalog has custom field types). See [UI](/docs/ui). Boolean `unsetOnOff` is a field key, not meta.
