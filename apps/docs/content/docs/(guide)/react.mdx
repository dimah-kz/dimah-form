---
title: React Client & useFormResponse
description: Headless questionnaire state, dynamic field visibility, autosave, and submission with useFormResponse.
icon: PenLine
---

`@dimah-form/react` provides a headless, reactive runtime for questionnaire forms. It manages the entire lifecycle of a response session—tracking visible fields, handling user input, debouncing draft saves, validating against the snapshot, and managing submit state—without imposing any UI components or styling.
