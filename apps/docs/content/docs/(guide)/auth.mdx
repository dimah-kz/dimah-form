---
title: Authentication & Lifecycle Hooks
description: Secure endpoints with guard, protect respondent records, and execute pre/post-persistence lifecycle hooks.
icon: Shield
---

In **dimah-form**, authentication and authorization live entirely in the consumer-provided **`guard`** hook. Persistence hooks (`on*` and `after*`) let you enrich records and trigger side-effects like emails or webhooks.
