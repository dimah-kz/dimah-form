---
title: Headless UI & Widget Binding
description: Connect form state to your design system using FormFieldBinding, widget dispatchers, and multi-step steppers.
icon: Blocks
---

**dimah-form** is intentionally headless: it does not ship with any bundled CSS or pre-styled UI widgets. Instead, it exposes a typed binding interface (`FormFieldBinding`) that lets you connect any input component—from standard HTML `<input>` tags to shadcn/ui, Radix, or custom design system primitives.
