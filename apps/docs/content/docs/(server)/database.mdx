---
title: Database
description: Persistence is required. A SQL database is not.
icon: Database
---
...
| :---------------- | :------------------- | :----------------------------------------------- |
| `memoryAdapter()` | `@dimah-form/server` | Tests, local, [Quickstart](/docs/quickstart)     |
| `db(formDb)`      | `@dimah-form/db`     | Production (FumaDB + Drizzle, Prisma, or Kysely) |
| Custom            | your code            | Implement [`ResponseStore`](/docs/configuration) |
...
Implement [`ResponseStore`](/docs/configuration) and pass it to `dimahForm({ database })` directly. Do not wrap it in `db()`.
