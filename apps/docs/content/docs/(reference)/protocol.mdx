...
| `GET`  | `/responses`        | `listResponses({ formId?, respondentId?, status?, submittedFrom?, submittedTo?, updatedAfter?, limit?, offset? })` | `listResponses({ query })`  | List response summaries (or full rows with `include=full`). `total` is the matching count. |
