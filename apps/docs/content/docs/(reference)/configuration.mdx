...
| **`validateDefinition`** | `DefinitionValidator`                 | No       | `undefined`   | Whole-document checks after plugin `metaSchema`. Plugin callbacks run first, then this function. Synchronous. Init, `saveForm`, and live form reads — not response snapshots. |
| **`plugins`**            | `DimahFormPlugin[]`                   | No       | `[]`          | Server plugins created with `definePlugin()`. Merge endpoints, hooks, field types, error codes, `metaSchema`, `validateAnswers`, and `validateDefinition`.                    |
...
| **`plugins`**     | `FormClientPlugin[]`                               | `[]`               | Companion client plugins created with `defineClientPlugin()`. Pass `createFormClient<Form, typeof plugins>({ plugins })` for literal endpoint names. |
