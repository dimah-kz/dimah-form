...
`createDefineForm({ fieldTypes })` is type-only — still pass the same array to `dimahForm({ fieldTypes })` and `createFormClient({ fieldTypes })`. TypeScript infers `Form["$Infer"]["answers"]["review"]["score"]` as `number`.
