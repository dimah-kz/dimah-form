...
`createDefineForm({ fieldTypes })` is type-only — still pass the same array to `dimahForm({ fieldTypes })` and `createFormClient({ fieldTypes })`. It types the field document (`min` / `max` here). `satisfies FormDefinitionUi<typeof customFieldTypes>` is only for `@dimah-form/ui` `meta` keys; drop it if you do not use that package. TypeScript infers `Form["$Infer"]["answers"]["review"]["score"]` as `number`.
