...
Unknown `add` variables, mixed `points`/`add`, and `field.variable` plus `option.add` fail `validateDefinition` (`SCORING_UNKNOWN_VARIABLE`, `SCORING_INVALID_META`, `SCORING_OPTION_ADD_MIX`).

### Reverse scoring

`reversed = min + max - points`.

- **select / multiSelect** — min/max are **that field's** option points (a 0–3 item on a 0–21 scale reverses as `3 − points`). `option.add` is not reversed.
...
Malformed mapping uses stable codes such as `SCORING_UNKNOWN_VARIABLE`, `SCORING_MISSING_POINTS`, and `SCORING_OPTION_ADD_MIX` (`form.$ERROR_CODES`).
