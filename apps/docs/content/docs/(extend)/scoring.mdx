...
Optional typed sums — not a formula language, and never `eval`:
...
Guard operation for `GET /scoring/response` is `getResponseScores`. Document validity (unknown variables, missing option points) is checked at `dimahForm()` / `saveForm` via `metaSchema` + `validateDefinition`. Scoring is not answer validation.
