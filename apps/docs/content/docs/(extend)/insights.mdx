...
- optional scoring band / mean / stdev when `@dimah-form/scoring` is installed. Bands follow `meta.scoring` order, including unused `n: 0`
...
`summary.byStatus` is the census. `completion` uses visible required fields on **each row’s snapshot**. `fields` follow the live questionnaire order; ids that exist only on older snapshots come after. `scores` is omitted when `@dimah-form/scoring` is missing or the snapshot has no `meta.scoring`. A crosstab axis that is on the live form and not categorical is `VALIDATION_ERROR`. An id that is only on older snapshots still folds (empty when no row has it). `table.n` is the respondent base for the cells.
