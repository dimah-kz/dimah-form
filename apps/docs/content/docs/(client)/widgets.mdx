...
On the headless path, filter `q.visibleFields` by `meta.step` yourself — see [UI](/docs/ui). Type-check UI `meta` with `import type { FormDefinitionUi } from "@dimah-form/ui"` and `defineForm({ ... } satisfies FormDefinitionUi)`.
