...
Missing items default to `"incomplete"` (`raw: null`). Reverse scoring is `min + max - points` using that field's option range for select items. Bands are interpretation. Optional typed `formulas: [{ op: "sum", vars }]` can combine subscales. Formula `vars` must be variable ids — not other formulas.
