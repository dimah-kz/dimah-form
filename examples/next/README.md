...
- `GET /api/forms/:formId/package` is a **consumer** download route. JSONL streams `createDatasetReader` + `toJsonlLine` and does not load the codebook. CSV, labels, and codebook use `getDatasetCodebook` first and return 409 when `truncated` is true. It is not a library zip
